...
### 快速排序
#### 基本流程
通过一趟排序将数据分割成独立的两部分，其中一部分所有的数据比另一部分所有的数据要小，再按照这个方法对这两部分数据进行快速排序，
整个排序过程可以递归进行

#### 原理
1、首先，需要选取一个元素作为基准数(哨兵)，通常选取第一个元素。

2、然后将待排序队列与基准数进行比较，小于基准数的元素移动到数列的左侧，大于基准数的元素移动到数列的右侧。

3、最后，接着把两个分区的元素按照上述的方法找到基准数，然后进行比较移动，直到各个分区只有一个元素为止。