...
- 先判断资源的分配情况，计算剩余资源数量
- 再判断进程的申请情况，判断资源是否足够
- 若资源足够则非阻塞节点，去掉进程所有的边（孤立操作）
- 只有请求资源才会发生死锁，若进程只有分配边，则直接去掉进程所有的边
...
- 逆向工程：分析程序，力图在源代码更高抽象层次上建立程序的表示过程，逆向工程是设计的恢复过程
...
- V模型：测试贯穿始终，测试分阶段，测试计划提前
- 喷泉模型：面向对象模型，特点是迭代、无间隙，各个阶段无明显界限，可以迭代交叉
...
- 构思阶段：定义最终产品视图和业务模型，确定系统范围
- 细分阶段：设计及确定系统架构，制定工作计划及资源要求
- 构建阶段：开发剩余构建和应用程序功能
- 移交阶段：确保软件对最终用户是可用的
...
### 软件需求层次
- 业务需求：组织机构、客户、产品高层次的目标要求，通常来自项目投资人、购买产品的客户等
- 用户需求：用户使用产品要完成的任务，表示用户的具体需求
- 功能需求：开发人员要实现的功能，包括功能需求、非功能需求和设计约束等
...
发展道路：初始阶段、扩展阶段、控制阶段、统一阶段、数据管理阶段、成熟阶段
