...
### 用例图
由参与者、用例、边界以及它们之间的关系组成的，描述整个系统的功能以及用户如何与系统进行交互

规约：用例名称、简要说明、事件流、非功能需求、前置条件和后置条件、拓展点、优先级
...
### 状态机图
从某个对象的状态是如何变化的角度来展示的，是一种由状态和迁移事件组成的状态机，
用来描述类的对象所有可能的状态以及时间变化时状态的转移条件

- 状态：起始状态（实心圆）、结束状态（黑点圆）、矩形
- 迁移：导致状态发生变化的事件，箭头指向变化后的状态

适用场景：某一个流程是围绕事物的状态变化而展开的

### 活动图
描述具体业务用例的实线流程，强调从活动到活动的控制流，可以表示并发执行的过程（流程图只能表示顺序执行的过程）

- 点：起始点（实心圆）、结束点（黑点圆）
- 活动：人或系统的一连串执行细节，矩形表示
- 迁移：表示流程控制权的迁移
- 分支：代表一个判断的准则，以菱形块表示
- 分叉和会合：代表对于后续活动的同步处理，分叉与会合会搭配出现
- 泳道：利用泳道可以分配对应的角色，可以帮助我们清晰地知道发起活动的角色是谁
...
### 认证技术
#### 数字签名
基于非对称加密的技术，用于验证数据的真实性和发送者的身份
- 身份认证：确保数据是由私钥持有者签名的
- 数据完整性：确保数据在传输过程中未被篡改
- 不可否认性：发送者无法否认自己签名过的内容，A的密钥信息作为防止A抵赖的证据

原理：
- 使用发送者的私钥对这个杂凑值进行加密，生成数字签名
- 接收者收到数据和数字签名后，使用发送者的公钥对数字签名解密获得杂凑值
- 接收者再对接收到的数据进行杂凑运算，得到另一个杂凑值，若两个杂凑值相同，则验证通过
...
#### 防火墙
类型
- 包过滤防火墙：工作在网络层，过滤流量，缺点是不检查数据包的内容，难以检测复杂攻击
- 状态检测防火墙：工作在传输层，记录连接状态，缺点是处理速度慢，占用更多资源
- 代理防火墙：工作在应用层，截获并验证流量然后转发或阻止访问请求，缺点是影响网络性能
...
- IPSec：用于VPN、站点到站点加密传输
- SSH：用于安全的远程登录、文件传输
- S/MIME和PGP：用于电子邮件加密和签名
- Kerberos：用于局域网中身份认证
- WPA2/WPA3：用于保护Wi-Fi通信
## 系统生命周期
...
- 系统分析：业务流程分析、系统可靠性分析
- 系统设计：结构化建模技术（数据流图）、面向对象建模技术（UML）
- 数据库建模：E-R模型、规范化、反规范化、并发控制和死锁、数据同步
- 项目管理：进程、范围、成本、质量、风险管理
- 其他
## 系统规划
### 可行性分析
...
## 需求分析
### 需求获取的方法
- 用户访谈：一对一直接与用户进行对话，关键利益相关者或领域专家
...
- 采样：选取特定的用户群体分析他们的行为或需求
  - 样本数量=0.25*（可信度因子/错误率）2次方
- 问卷调查：快速收集广泛群众的基本需求
- 阅读历史文档：有成熟的文档，适用于需要对系统进行升级或替换（已有系统）
- 现场观摩：理解用户行为和工作流程（已有系统）

### 需求评审
#### 内容
- 所有需求都被清楚的表达
- 需求规格说明书中的需求是从系统需求、业务规格和其他来源中正确推导而来的
- 需求是完整和高质量的
#### 作用
- 了解需求动机、目标和方案，降低需求的不完整
- 提早考虑排期风险
- 提高软件质量
- 统一团队成员理解
...
- 汇聚层：将网络业务连接到接入层，并且实施与安全、流量负载和路由相关的策略，为了保证核心层连接运行不同协议的区域，各种协议的转换都应在汇聚层完成
  ...
  - 灵活性和可扩展性
  - 连接多样性
- 接入层：为局域网接入广域网或者终端用户访问网络提供接入，接入层应负责一些用户管理功能
  - 用户接入
  - 网络访问控制
  - 服务与应用支持
  - 简化管理
