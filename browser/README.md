...
- https在http的基础上增加SSL加密传输协议，组成了以安全为目标的http通道；
- 需要CA申请证书，数字签名
...
1、初次请求：浏览器向服务器发起请求，服务器返回200，开始下载资源并缓存资源与响应头

2、Cache-Control字段：判断两次请求的时间差是否超过max-age，没有超过表示未过期，命中强缓存
...
- 1.1：
  ...
  - 加入缓存机制
  - 流控制基于tcp连接
  - 拥有长链接，支持请求管道化，可以依次发送所有要发送的请求，不必等待请求的响应
#### http2
...
##### 二进制传输
将原本header + body的格式拆分成数个二进制帧碎片，用header帧存放头信息，data帧存放实体信息

##### header压缩
在客户端和服务器两端建立字典，用索引号表示重复的字符串，采用哈夫曼编码来压缩整数和字符串

对于两个请求，比较出差异的数据，第二次发送请求只需要发送差异数据，减少冗余

对于新的键值对要么追加到表的尾部，要不替换表中原始的值

#### http3
建立在quic上运行，用quic替代tcp，解决队头拥塞问题